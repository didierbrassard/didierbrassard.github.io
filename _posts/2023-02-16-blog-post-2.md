...
measurement using a linear regression model. This measurement error
model allows to obtain a correction factor which can then be used to
calibrate the association of interest.
...
$Y_i=\beta_0 + \beta_{X_i} X_i +\epsilon_i$. However, a naive analysis
ignoring random errors would rather have the estimand $E(Y|W_j)$ and
model equation $Y_i=\beta\prime_0 + \beta\prime_{W_i} W_i +\epsilon_i$ .
...
### Regression coefficient of $\beta\prime_{W_j}$ vs. $\beta_X$
...
### Standard error of $\beta\prime_{W_j}$ vs. $\beta_X$
...
$$ \beta_X = \beta\prime_{W_J}/\lambda$$ which reads as: the *unbiased*
(calibrated) regression coefficient of the association between
vegetables and fruits and LDL cholesterol $\beta_X$ equals to the
*biased* (naive) regression coefficient obtained with measured
vegetables and fruits on a given day $\beta\prime_{W_J}$ divided by the
...
For the current demonstration, there are no covariates $Z_i$ and the
...
coefficient should be -0.102, consistent with the true association!
...
software. However, these methods are currently available in `SAS` only.
