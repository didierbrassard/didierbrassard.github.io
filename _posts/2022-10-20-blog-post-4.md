...
unfortunate because the bootstrap is (relatively) simple and extremely useful.
...
the original dataset. In other words, the idea is to repeat the sampling
process that led to the original sample to create many new samples.
These new samples then allow us to estimate standard errors or
confidence intervals.
...
automobiles (1973–74 models*”) and R code. Say that this data is a simple random
sample of cars available at the time. We might be interested in drawing
inference about the horsepower of these cars.
...
<img src="/images/2022-10-20-blog-post-4_files/figure-gfm/preparation-1.png" width="80%" style="display: block; margin: auto;" />
...
how we obtained the original sample.
...
simple random sampling) is consistent with the original sample **and**
...
mtcars_rows1 <-
  ...
  ## See which rows were selected
  ggplot(data=mtcars_rows1, aes(value)) +
    geom_dotplot() + 
    scale_x_continuous(limits=c(1,32),breaks=seq(1,32,1)) +
    scale_y_continuous(NULL, breaks = NULL) +
    labs(title = "Random sample (with replacement) of cars in mtcars",
         x="Car ID from the original sample",
         caption="Stacked dots indicate >1 selection for a given id") +
    theme_bw()
```

<img src="/images/2022-10-20-blog-post-4_files/figure-gfm/resampling-1.png" width="80%" style="display: block; margin: auto;" />

Using one resample, we find that some cars were picked more than once
(stacked dots in the figure), while others were not (empty space). The
mean horsepower in this resample is 136, while the mean horsepower is
147 HP in the original sample. Informally, we can interpret this
bootstrap sample as just another sample that we could have gotten, had
we sampled cars again from the hypothetical larger population of cars.

Of note, there is only one sample in the `mtcars` example. However, sometimes there are **two** samples as in the case of a randomized experiment with two interventions. Accordingly, we would have to perform simple random sampling **within each intervention arms** to create bootstrap samples based on such design (Efron and Tibshirani, 1994). All in all, the bootstrap resampling must be consistent with the original sampling.
...
in each of these new samples is very fast since in this example. Indeed,
the sample size is small (n=32) and the analysis is very simple (i.e.,
...
<img src="/images/2022-10-20-blog-post-4_files/figure-gfm/bootstrap-1.png" width="80%" style="display: block; margin: auto;" />
...
<img src="/images/2022-10-20-blog-post-4_files/figure-gfm/bootstrap-2.png" width="80%" style="display: block; margin: auto;" />
...
95%CI shows that the data are compatible with a median difference of 46
to 126.
