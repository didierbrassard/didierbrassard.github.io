‘Talented fighters are lazy!’: uncovering selection bias in combat
sports
================
...
first, some athletes were picking up techniques and moves more naturally
than others. In turn, those athletes deemed more naturally talented also
...
## An informal explanation of *collider-stratification bias*
...
a **FIGHTER**), the practitioner has to either have **TALENT** (e.g.,
natural athletic abilities) or **WORK** ethic (e.g., never miss
practice). This is depicted in the figure below, a causal graph.

<img src="2022-09-15-blog-post-1_files/figure-gfm/dag-selection-bias-1.png" width="75%" style="display: block; margin: auto;" />

Assuming that my causal graph is correct, **FIGHTER** is a collider;
both arrows from **TALENT** and **WORK** collide into being a
**FIGHTER**. The fact of looking only at those that will eventually step
in the ring is - in epidemiological jargon - is defined as conditioning
on **FIGHTER** (or stratifying by FIGHTER). Indeed, among all people
that walk through the gym doors, we unconsciously condition (or
stratify) for individuals that become **FIGHTER**. It is indeed easier
to remember individuals that spent months and years training and
competing rather than others that only practiced for a short period. And
this is the key to collider-stratification bias: fighters are not a
sample of individuals representative of *all* the population in terms of
natural talent and work ethic. I will come back to this point in the
next sections.

Of note, one nuance in the graph is that there is initially no
relationship between **TALENT** and **WORK**. In other words, work ethic
does not cause talent and vice versa. For example, the fact that an
individual has a high proportion of explosive muscle fibres (a genetic
factor potentially contributing to perceived talent) should not greatly
affect one’s work ethic. On the contrary, if talent affected one’s work
ethic (**TALENT-\>WORK**), it could even push to work more (“*Oh I’m
good at this!*”-\>“*Let’s do more*”). So, for the purpose of this
...
work ethic. Inversely, work ethic cannot affect one’s genetic for
example.

## What is the impact of collider bias?

Using the model above, I can run a very simple simulation to show that,
once we look only at those that become **FIGHTERS**, **TALENT** will
...
over time. In reality, this effect does not exist and is a perfect
...
corr_talent_work <- cor(talent,work)
corr_talent_work
```
...
# Visualize this cut-off 
 ggplot(data=population,aes(x=total),stat="identity") +
  ...
  labs(title = paste0("Distribution of natural talent and work ethic in ",n," combat sport practitioners"),
       x="Talent + work score",y="Proportion of population",
       caption="Individuals to the right of the dashed line are fighters") +
  theme_bw()
```

<img src="2022-09-15-blog-post-1_files/figure-gfm/combination-1.png" width="75%" style="display: block; margin: auto;" />
...
Having selected the top 10% individuals based on their combined
**talent** + **work** score, we can look at the correlation between
...
**WORK** is -0.6! Recall that in our overall population, the same
correlation was 0.06. Clearly, when looking only at fighters, i.e., the
top athlete that will eventually step in the ring, natural **TALENT**
seems negatively correlated with **WORK** ethic. The common
interpretation is that talent somehow causes fighters to be lazy.
...
<img src="2022-09-15-blog-post-1_files/figure-gfm/fighter-highlight-1.png" width="75%" style="display: block; margin: auto;" />
