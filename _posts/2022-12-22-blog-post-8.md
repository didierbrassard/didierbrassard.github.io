...
statistical software. For example, estimating mean a diet quality score
based on 24-hour dietary recall, the main dietary assessment instrument
in surveys, is one example of such analysis. The recommended approach is
the **population ratio method** (Freedman et al. 2008).

The goal of this post is to save you some time and provide you with code
explaining how to account for the CCHS 2015 design to calculate diet
quality score using the population ratio method. Note that data for the
CCHS 2015 - Nutrition (Public Use Micro Datafile or PUMF) are [publicly
available](https://www150.statcan.gc.ca/n1/en/catalogue/82M0024X2018001).

# Population ratio vs. simple scoring

Briefly, to derive a diet quality score, dietary intakes and ratio of
intakes are estimated at the population- or group-level. For example, to
score the percentage of total energy intake (%E) as saturated fats
(SFA):  
1. we calculate the mean calories from saturated fats in the overall
population or group of interest ($\mu_{SFA}$) and then the mean total
energy intake in calories ($\mu_{E}$).  
...
to derive the diet quality score of interest.

Alternatively, one could have used a **simple scoring** approach. In a
simple scoring approach, the scoring algorithm is directly applied to
each respondent’s dietary intakes $f(\frac{SFA_i}{E_i})$. Each
respondent has a score and the mean of these scores is then used to
estimate the mean diet quality score of the population or group:
$\mu_f{(\frac{SFA_i}{E_i})}$. Unfortunately, this method is less optimal
since it assumes that each respondent’s dietary intakes reflect usual
dietary intakes, i.e., a long-term average. This is not the case when
using only one 24-h dietary intake, because of random errors ([more
details
here](https://didierbrassard.github.io/posts/2022/11/blog-post-6/)). On
the contrary, the population ratio method directly scores usual dietary
intakes because mean intakes at the group level do reflect usual dietary
intakes.
...
vs. non-smokers as well as score difference). The HEFI-2019 score
reflect the adherence to Canada’s Food Guide 2019’s recommendations on
healthy food choices (Brassard et al. 2022a, b). The data used for the
present example are already “pre-processed” and complete details are
available in the [HEFI-2019 Github
...
Typically, we would use built-in function (e.g., with the `survey`
package in R) to obtain estimates and confidence interval. However,
there is rarely a built-in function for multistep analyses. In the
...
through all bootstrap replicate weights (i.e., obtain mean estimates for
each bootstrap weight) and, then, calculate confidence intervals based
on the (bootstrap) sampling distribution.
...
lunching lengthy bootstap analyses. For this test, we will estimate
HEFI-2019 score based on the sampling weights (i.e., `BSW0`) without
variance.
...
corresponds to a 7.2 lower score. In other words, smokers have a lower
adherence to Canada’s Food Guide 2019 recommendations on healthy food
choices. This is expected since non-smokers tend to have a higher
propensity to health-seeking behaviors, including overall diet quality
and hence, adherence to Canada’s Food Guide 2019 recommendations.
...
intervals, e.g., 0.05 for 95% confidence interval. There are again
different options possible to calculate confidence intervals, including
normal approximation or percentile.For simplicity, the normal
approximation method is shown below.
...
# 2) Check bootstrap estimates normality
...
We can then use the `hefi2019smkf` data to generate table or graph with
results. I show a simple table below:
