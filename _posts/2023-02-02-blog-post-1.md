...
linearity assumption is not even required! We can relax the linearity
assumption by using simple statistical transformations. One of those
transformation is the restricted cubic spline.
...
**Restricted**\* means the spline is forced to be linear below and above
...
([Table 1](#tbl-rcs-quantile)). Using quantiles that are
...
<div id="tbl-rcs-quantile">

| Number of knots | Position                             |
|----------------:|:-------------------------------------|
|               3 | .10 .50 .90                          |
|               4 | .05 .35 .65 .95                      |
|               5 | .05 .275 .50 .725 .95                |
...
Table 1: Common knot quantiles for restricted cubic spline (Harrell,
2015)
...
model together with the original variable. For example, the original
model equation:
...
$Y_i=\beta_0+\beta_XX_i+\beta_X'X_i'+\beta_X''X_i''+\epsilon_i$
...
- $X_i'$ and $X_i''$ are derived from $X$ for the transformation.
  $\beta_X'$ and $\beta_X''$ are the coefficient reflecting the
  non-linear relationship, if any.

- $X_i$, $X_i'$ and $X_i''$ are variables that together reflect the
  independent variable. $\beta_X$, $\beta_X'$ and $\beta_X''$ are
  regression coefficients reflecting the full relationship between $X$
  and $Y$.
...
$$\text{PHYS_ACT_VIG}=\beta_0+\beta_{AGE}+\beta_{AGE}'+\beta_{AGE}''+\beta_{MALE}+\epsilon$$
...
$\beta_{AGE}$, $\beta_{AGE}'$ and $\beta_{AGE}''$ mean? Once a
restricted cubic spline transforamtion is applied, we interpret the
graph, not the coefficients!
...
id="fig-lm-rcs" style="width:80.0%" data-fig-align="center"
...
id="fig-lm-norcs" style="width:80.0%" data-fig-align="center"
